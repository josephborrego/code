...
void bench(unsigned keyrange, unsigned iters, unsigned buckets, unsigned ratio, unsigned nthreads)
{
    ...
    for (int i = 0; i < keyrange; i += 2) my_set.insert(i);
    ...
    auto task = [&](int id)
	{
        ...
        for (int i = 0; i < iters; ++i) 
		{
            ...
            if (action < ratio) my_set.lookup(val);
            else if (action < ratio + (100 - ratio)/2) my_set.insert(val);
            else my_set.remove(val);
        }
    ...
    for (int i = 0; i < nthreads; ++i) threads.push_back(std::thread(task, i));
    for (int i = 0; i < nthreads; ++i) threads[i].join();

    // print the benchmark throughput
    auto elapsed = duration_cast<duration<double>>(end_time - start_time).count();
...
int main(int argc, char** argv)
{
    ...
    if (test == 'l') bench<clist>(keyrange, ops, buckets, ratio, threads);
    else if (test == 'r') bench<rwlist>(keyrange, ops, buckets, ratio, threads);
    else if (test == 'h') bench<chash>(keyrange, ops, buckets, ratio, threads);
    else if (test == 's') bench<shash>(keyrange, ops, buckets, ratio, threads);
    exit(0);
}
