...
void bench(unsigned keyrange, unsigned iters, unsigned buckets, unsigned ratio, unsigned nthreads, unsigned ops) {
    ...
    auto task = [&](int id) 
    {
        ...
        for (int i = 0; i < iters; ++i) {
            ...
            if (action < ratio) my_set.lookup(vals, res, ops);
            else if (action < ratio + (100 - ratio)/2)  my_set.insert(vals, res, ops);
            else my_set.remove(vals, res, ops);
        }
...
int main(int argc, char** argv) 
{
    ...
    if (test == 'h') { bench<shash2>(keyrange, iters, buckets, ratio, threads, ops); }
    else if (test == 't') { bench<tree>(keyrange, iters, buckets, ratio, threads, ops); }
    exit(0);
}
